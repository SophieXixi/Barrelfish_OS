...
/**
 * Reads the MBR information of the SD card and extracts partition information
 *
 * \param sd        The driver struct
 * \param scratch   Scratch region virtual address
 * \param scratch_p Scratch region physical address
 * \param startlba  Returns the start LBA of the first partition
 * \param nblocks   Returns the number of blocks of the first partition
 *
 */
errval_t sdhc_read_mbr(struct sdhc_s* sd, void * scratch, lpaddr_t scratch_p,
                       uint32_t *startlba, uint32_t *nblocks);
