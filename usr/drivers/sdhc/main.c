...
static sdhc_cmd_xfr_typ_t xfr_typ_for_cmd(struct cmd *cmd)
{
    ...
    if (cmd->cmdidx == MMC_CMD_READ_SINGLE_BLOCK || cmd->cmdidx == MMC_CMD_WRITE_SINGLE_BLOCK) {
...
{
    errval_t   err;
    struct cmd init = { .cmdidx = MMC_CMD_GO_IDLE_STATE, .cmdarg = 0, .resp_type = MMC_RSP_NONE };
...
static errval_t sdhc_send_if_cond(struct sdhc_s *sd)
{
    ...
    struct cmd cmd     = { .cmdidx    = SD_CMD_SEND_IF_COND,
...
static errval_t sdhc_get_ocr(struct sdhc_s *sd)
{
    ...
    while (1) {
        struct cmd app_cmd = { .cmdidx = MMC_CMD_APP_CMD, .cmdarg = 0, .resp_type = MMC_RSP_R1 };
        ...
        struct cmd op_cond_cmd = { .cmdidx    = SD_CMD_APP_SEND_OP_COND,
...
static errval_t sdhc_card_init(struct sdhc_s *sd)
{
    ...
    struct cmd cid = { .cmdidx = MMC_CMD_ALL_SEND_CID, .resp_type = MMC_RSP_R2, .cmdarg = 0 };
    ...
    struct cmd send_addr
        = { .cmdidx = SD_CMD_SEND_RELATIVE_ADDR, .cmdarg = sd->rca << 16, .resp_type = MMC_RSP_R6 };
    ...
    struct cmd send_csd
        = { .cmdidx = MMC_CMD_SEND_CSD, .resp_type = MMC_RSP_R2, .cmdarg = sd->rca << 16 };
    ...
    struct cmd select_card
        = { .cmdidx = MMC_CMD_SELECT_CARD, .resp_type = MMC_RSP_R1, .cmdarg = sd->rca << 16 };
...
errval_t sdhc_read_block(struct sdhc_s *sd, int index, lpaddr_t dest)
{
    ...
    struct cmd set_blocklen
        = { .cmdidx = MMC_CMD_SET_BLOCKLEN, .cmdarg = SDHC_BLOCK_SIZE, .resp_type = MMC_RSP_R1 };
    ...
    struct cmd read_block = { .cmdidx    = MMC_CMD_READ_SINGLE_BLOCK,
                              .cmdarg    = index,
...
errval_t sdhc_write_block(struct sdhc_s *sd, int index, lpaddr_t source)
{
    ...
    struct cmd write_block = { .cmdidx    = MMC_CMD_WRITE_SINGLE_BLOCK,
                               .cmdarg    = index,
                               .resp_type = MMC_RSP_R1,
                               .dma_base  = source
...
errval_t sdhc_test(struct sdhc_s *sd, void *scratch, lpaddr_t scratch_p)
{
    ...
    debug_printf("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
    debug_printf("PERFORMING SDHC TEST -- This may corrupt the disk!");
    debug_printf("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
...
errval_t sdhc_read_mbr(struct sdhc_s *sd, void *scratch, lpaddr_t scratch_p, uint32_t *startlba,
                       uint32_t *nblocks)
{
    errval_t err;

    // read block 0 that contains the MBR sector
    err = sdhc_read_block(sd, 0, scratch_p);
    if (err_is_fail(err)) {
        DEBUG_ERR(err, "read block");
        return err;
    }
    ...
    uint8_t *mbr = (uint8_t *)scratch;

    #ifdef DEBUG_ON
    printf("Dumping MBR sector.\n");
    printf("-----------------------------------------------------\n");
    for (size_t i = 0; i < 512 / 16; i++) {
        printf("[%03x] %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x "
               "%02x\n",
               i * 16, mbr[i * 8 + 0], mbr[i * 16 + 1], mbr[i * 16 + 2], mbr[i * 16 + 3],
               mbr[i * 16 + 4], mbr[i * 16 + 5], mbr[i * 16 + 6], mbr[i * 16 + 7], mbr[i * 16 + 8],
               mbr[i * 16 + 9], mbr[i * 16 + 10], mbr[i * 16 + 11], mbr[i * 16 + 12],
               mbr[i * 16 + 13], mbr[i * 16 + 14], mbr[i * 16 + 15]);
    }
    printf("-----------------------------------------------------\n");
    #endif
    ...
    if (mbr[0x01FE] != 0x55 || mbr[0x01FF] != 0xAA) {
        printf("Signature not found: %02X %02X expected 0x55 0xAA\n", mbr[0x01FE], mbr[0x01FF]);
        return -1;  // TODO: DEFINE ERROR CODE FOR THIS.
    }

    uint32_t start_lba;
    uint32_t num_blocks;

    // fat and mbr have the same signature. fat defines bytes 0 and 2 to be 0xEB and 0x90,
    // so we can use this to differentiate. However, this may not be sufficient as some mbr
    // may also contain these bytes.
    if (mbr[0] == 0xEB && mbr[2] == 0x90) {
        start_lba  = 0;
        num_blocks = *(uint32_t *)(mbr + 32);
    } else {
        // the partition table is not aligned, so we create an aligned copy of it
        uint32_t partition_table_entry[4] = { 0 };
        memcpy(partition_table_entry, &mbr[0x01BE], 16);

        start_lba  = partition_table_entry[2];
        num_blocks = partition_table_entry[3];
    }
    ...
    printf("Size of Partition: %zu MB\n", ((size_t)num_blocks * 512UL) >> 20);
    #endif
