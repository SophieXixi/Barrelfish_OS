...
if ${ENABLE_NETWORKING}; then
    NETWORKING_CONFIG="-netdev tap,id=network0 -device virtio-net-device,netdev=network0"
    NETWORKING_CONFIG="-netdev user,id=network0,hostfwd=udp:127.0.0.1:55555-192.168.100.1:50 -device virtio-net-device,netdev=network0"

    echo "enabling networking requires sudo"
    # QEMU_PATH="sudo ${QEMU_PATH}"
else
    NETWORKING_CONFIG=""
    echo "not enabling networking"
fi
