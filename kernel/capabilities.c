...
static errval_t set_cap(struct capability *dest, struct capability *src)
{
    ...
    // When forging caps, the source is not in the CSPace but on the userspace stack, don't try to print CTE
    debug(SUBSYS_CAPS, "Copying cap from %#"PRIxLPADDR" to %#"PRIxLPADDR"\n",
            (lvaddr_t)src >= MEMORY_OFFSET ? mem_to_local_phys((lvaddr_t)cte_for_cap(src)) : 0x0,
                            ...
                            struct cte *dest_caps)
{
    ...
    (void)is_ept; // make compiler happy
