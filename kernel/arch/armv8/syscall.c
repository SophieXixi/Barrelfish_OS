...
void sys_syscall(union syscall_info si, uint64_t a1, uint64_t a2, uint64_t a3,
    ...
    )
{
    ...
    capaddr_t root  = sa->arg1;
    uint8_t   level = sa->arg2;
    capaddr_t vptr  = sa->arg3;
    capaddr_t dptr  = sa->arg4;
    bool      run   = sa->arg5;
    capaddr_t odptr = sa->arg6;
    ...
    )
{
    ...
    enum task_type type = (enum task_type)(sa->arg2 >> 16);
    uint16_t weight = sa->arg2 & 0xffff;

    return sys_dispatcher_properties(to, type, sa->arg3,
                                     sa->arg4, sa->arg5, sa->arg6, weight);
}
...
{
    /* Retrieve arguments */
    capaddr_t dest_cspace_cptr = args->arg1;
    capaddr_t destcn_cptr      = args->arg2;
    uint64_t  dest_slot        = args->arg3;
    capaddr_t source_croot_ptr = args->arg4;
    capaddr_t source_cptr      = args->arg5;
    uint8_t destcn_level       = args->arg6;
    uint8_t source_level       = args->arg7;
    ...
    )
{
    ...
    capaddr_t dest_cspace_cptr = sa->arg7;
    // Destination cnode cptr
    capaddr_t dest_cnode_cptr  = sa->x8;
    // Destination slot number
    capaddr_t dest_slot        = sa->x9;
    ...
    )
{
    ...
    capaddr_t cptr = (capaddr_t)sa->arg1;
    int     bits = (int)sa->arg2;
    ...
    )
{
    ...
    enum objtype type      = sa->arg1;
    size_t       objsize   = sa->arg2;
    capaddr_t    dest_cptr = sa->arg3;
    uint8_t      dest_level= sa->arg4;
    cslot_t      dest_slot = sa->arg5;
    ...
    )
{
    ...
    capaddr_t newroot_ptr = sa->arg1;
    capaddr_t retcn_ptr   = sa->arg2;
    cslot_t   retslot     = sa->arg3;
    ...
    )
{
    ...
    /* Retrieve arguments */
    capaddr_t source_root_cptr = (capaddr_t)sa->arg1;
    capaddr_t source_cptr      = (capaddr_t)sa->arg2;
    uintptr_t flags            = (uintptr_t)sa->arg3;
    uintptr_t offset           = (uintptr_t)sa->arg4;
    uintptr_t pte_count        = (uintptr_t)sa->arg5;
    capaddr_t mcn_root         = (capaddr_t)sa->arg6;
    ...
    )
{
    ...
    /* Retrieve arguments */
    capaddr_t  mapping_cptr  = (capaddr_t)sa->arg1;
    int mapping_bits         = (int)sa->arg2 & 0xff;
        ...
        )
{
    ...
    // unpack arguments
    size_t offset = sa->arg1; // in pages; of first page to modify from first
                             // page in mapped region
    size_t pages  = sa->arg2; // #pages to modify
    size_t flags  = sa->arg3; // new flags
...
{
    (void)kernel_cap;
    INVOCATION_PRELUDE(2);
    // check access to user pointer
    if (!access_ok(ACCESS_READ, sa->arg1, sizeof(struct capability))) {
        return SYSRET(SYS_ERR_INVALID_USER_BUFFER);
    }

    struct capability *src = (struct capability *)sa->arg1;
...
INVOCATION_HANDLER(monitor_handle_delete_last)
{
    ...
    capaddr_t root_caddr   = sa->arg1;
    uint8_t   root_level   = sa->arg2;
    capaddr_t target_caddr = sa->arg3;
    uint8_t   target_level = sa->arg4;
    capaddr_t retcn_caddr  = sa->arg5;
    uint8_t retcn_level    = sa->arg6;
    cslot_t retcn_slot     = sa->arg7;
...
INVOCATION_HANDLER(monitor_handle_delete_foreigns)
{
    ...
    capaddr_t caddr = sa->arg1;
    uint8_t bits = sa->arg2;
...
INVOCATION_HANDLER(monitor_handle_revoke_mark_tgt)
{
    ...
    INVOCATION_PRELUDE(5);
    capaddr_t root_caddr   = sa->arg1;
    uint8_t root_vbits     = sa->arg2;
    capaddr_t target_caddr = sa->arg3;
    uint8_t target_vbits   = sa->arg4;
...
INVOCATION_HANDLER(monitor_handle_revoke_mark_rels)
{
    ...
    struct capability *base = (struct capability*)sa->arg1;
...
INVOCATION_HANDLER(monitor_handle_delete_step)
{
    (void)kernel_cap;
    INVOCATION_PRELUDE(4);
    capaddr_t ret_cn_addr = sa->arg1;
    capaddr_t ret_cn_bits = sa->arg2;
    capaddr_t ret_slot    = sa->arg3;
...
INVOCATION_HANDLER(monitor_handle_domain_id)
{
    (void)kernel_cap;
    INVOCATION_PRELUDE(3);
    capaddr_t cptr       = sa->arg1;
    domainid_t domain_id = sa->arg2;
...
INVOCATION_HANDLER(monitor_get_cap_owner)
{
    (void)kernel_cap;
    INVOCATION_PRELUDE(5);
    capaddr_t root_addr = sa->arg1;
    uint8_t root_bits   = sa->arg2;
    capaddr_t cptr      = sa->arg3;
    uint8_t bits        = sa->arg4;
    ...
    )
{
    ...
    capaddr_t ep_caddr = (capaddr_t)sa->arg1;
...
INVOCATION_HANDLER(monitor_cap_has_relations)
{
    ...
    INVOCATION_PRELUDE(4);
    capaddr_t caddr = sa->arg1;
    uint8_t vbits = sa->arg2;
    uint8_t mask = sa->arg3;
...
INVOCATION_HANDLER(monitor_remote_relations)
{
    ...
    INVOCATION_PRELUDE(6);
    capaddr_t root_addr = sa->arg1;
    int root_bits       = sa->arg2;
    capaddr_t cptr      = sa->arg3;
    int bits            = sa->arg4;
    uint8_t relations   = sa->arg5 & 0xFF;
    uint8_t mask        = (sa->arg5 >> 8) & 0xFF;
    ...
    )
{
    ...
    capaddr_t cnode_cptr = sa->arg1;
    int cnode_vbits      = sa->arg2;
    size_t slot          = sa->arg3;
    coreid_t owner       = sa->arg4;
...
INVOCATION_HANDLER(monitor_get_platform)
{
    ...
    INVOCATION_PRELUDE(2);
    // check args
    if (!access_ok(ACCESS_WRITE, sa->arg1, sizeof(struct platform_info))) {
        return SYSRET(SYS_ERR_INVALID_USER_BUFFER);
    }

    platform_get_info((struct platform_info*)sa->arg1);
...
INVOCATION_HANDLER(monitor_reclaim_ram)
{
    ...
    capaddr_t ret_cn_addr  = sa->arg1;
    capaddr_t ret_cn_level = sa->arg2;
    capaddr_t ret_slot     = sa->arg3;
    ...
    int argc)
{
    ...
    hwid_t core_id         = sa->arg1;
    enum cpu_type cpu_type = sa->arg2;
    genvaddr_t entry       = sa->arg3;
    genpaddr_t context_id  = sa->arg4;
    uint64_t psci_use_hvc  = sa->arg5;
    ...
    int argc)
{
    ...
    capaddr_t cptr = sa->arg1;
    int bits = sa->arg2;
    struct capability *retbuf = (void *)sa->arg3;
...
INVOCATION_HANDLER(monitor_identify_domains_cap)
{
    ...
    capaddr_t root_caddr = sa->arg1;
    capaddr_t root_vbits = sa->arg2;
    capaddr_t cptr       = sa->arg3;
    int bits             = sa->arg4;
    struct capability *retbuf = (void *)sa->arg5;
...

{
    ...
    (void)context;
    struct registers_aarch64_syscall_args* sa = &context->syscall_args;
    return SYSRET(irq_connect(to, sa->arg1));
}
    ...
    struct capability* to, arch_registers_state_t* context, int argc)
{
   ...
   struct registers_aarch64_syscall_args* sa = &context->syscall_args;
   paging_dump_tables(dispatcher, sa->arg1);
                                         ...
                                         arch_registers_state_t *context, int argc)
{
    ...
    capaddr_t cptr = sa->arg1;
    uint8_t level = sa->arg2;
    struct capability *cap = (void *)sa->arg3;
                                         ...
                                         int argc)
{
    ...
    capaddr_t cnode_cptr = sa->arg1;
    capaddr_t cnode_level = sa->arg2;
    uint16_t slot = sa->arg3;

    uint32_t address = sa->arg4;
    uint32_t segflags = sa->arg5;
                                  ...
                                  int argc)
{
    ...
    return sys_handle_kcb_identify(to, (struct frame_identity *)sa->arg1);
}
...
static struct sysret
handle_invoke(union syscall_info si, uint64_t a1, uint64_t a2, uint64_t a3,
              uint64_t a4, uint64_t a5, uint64_t a6,
              arch_registers_state_t *context)
{
    struct registers_aarch64_syscall_args* sa = &context->syscall_args;
    ...
    //
    uint8_t   flags       = si.invoke.flags;
    uint8_t   invoke_bits = si.invoke.invoke_level;
    capaddr_t invoke_cptr = si.invoke.invoke_cptr;
...
// printf("%s: %zd %zd\n", __func__, systime_now(), armv8_CNTP_TVAL_EL0_rd(NULL));
    ...
    if (ObjType_EndPointLMP == to->type && !(flags & LMP_FLAG_IDENTIFY))
    {
        ...
        if (listener->disp) {
            ...
            uint8_t send_bits    = (sa->x9 >> 32) & 0xf;
            capaddr_t send_cptr  = sa->x9 & 0xffffffff;
            ...
            // XXX: we should pass them to lmp_deliver() directly
            sa->arg0 = si.raw;
            sa->arg1 = a1;
            sa->arg2 = a2;
            sa->arg3 = a3;
            ...
            r.error = lmp_deliver(to, dcb_current, &context->regs[1],
    ...
    {
        uint8_t cmd = si.invoke.cmd;;
        int argc    = si.invoke.sc.argc;;
        if (cmd < CAP_MAX_CMD)
        {
...
//    debug(SUBSYS_SYSCALL, "syscall: disabled=%d\n", disabled);
    ...
    {
        case SYSCALL_INVOKE:
            r = handle_invoke(si, a1, a2, a3, a4, a5, a6, context);
            break;
